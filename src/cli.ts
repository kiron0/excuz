...
import { DEFAULT_LANGUAGE, isValidLanguage } from "./type.js";
import {
  COLORS,
  ERROR_MESSAGES,
  getAllExcuses,
...
program
  ...
  .action((options: { lang?: string }) => {
    try {
      ...
      process.stdout.write(`${COLORS.GREEN}${excuse.text}${COLORS.RESET}\n`);
    } catch (error) {
...
program
  ...
  .option("-l, --lang <language>", "Language code (bn|en)")
  .option("--limit <number>", "Limit the number of excuses to display")
  .action(function (this: any) {
    try {
      const options = this.parent?.opts() || program.opts();
      const lang = handleLanguageOption(options.lang);
      ...
      excuses.forEach((excuse, index) => {
        process.stdout.write(`${index + 1}. ${excuse.text}\n`);
      });
    } catch (error) {
      const options = this.parent?.opts() || program.opts();
      handleError(error, options?.lang);
...
program
  ...
  .action(function (this: any) {
    try {
      ...
      process.stdout.write(`Number of excuses in ${langName}: ${count}\n`);
    } catch (error) {
      const options = this.parent?.opts() || program.opts();
      handleError(error, options?.lang);
    }
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      if (action === "random") {
        const excuse = getRandomExcuse(lang);
        p.outro(`${COLORS.GREEN}${excuse.text}${COLORS.RESET}`);
      } else if (action === "list") {
        ...
        p.outro(
          `\n${displayExcuses.map((e, i) => `${i + 1}. ${e.text}`).join("\n")}${
            remaining > 0
